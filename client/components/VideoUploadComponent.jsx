...
function VideoUploadComponent() {
  ...
  const handleDownload = (file) => {
    const url = URL.createObjectURL(
      new Blob([file.data.buffer], { type: `video/${selectedFormat}` })
    );
        ...
        <div className="video-upload-container">
          <div className="container">
            <div
              {...getRootProps({
                className:
                  'dropzone  bg-blue-100 rounded-3xl border-4 border-dashed border-blue-500',
              })}
            >
              <input {...getInputProps()} />
            ...
            {files.length > 0 && (
              <div className="progress-bar">
                <div
                  style={{ width: `${uploadProgress[files[0].name]}%` }}
                ></div>
              </div>
            )}
              ...
              <div>
                ...
                {Object.keys(uploadedFiles).map((key) => (
                  <div key={key}>
                    ...
                    <video controls>
                      <source
                        src={URL.createObjectURL(
                          new Blob([uploadedFiles[key].data.buffer], {
                    ...
                    <button onClick={() => handleDownload(uploadedFiles[key])}>
